...
export async function scrubAttachment(
  ...
  if (isPdf) {
    ...
    return scrubPdf(bytes, async (text) => {
      const findings = scan(text).findings;
      if (findings.some((f) => f.severity === 'block')) throw new Error('Classification markings detected. Upload blocked.');
      const merged = resolveOverlaps([...findings, ...detectAttachmentEntities(text), ...await detectNames(text)]);
  ...
  const findings = scan(text).findings;
  if (findings.some((f) => f.severity === 'block')) throw new Error('Classification markings detected. Upload blocked.');
  const result = redact(text, resolveOverlaps([...findings, ...detectAttachmentEntities(text), ...await detectNames(text)]));
  ...
  return { file: new File([output], `scanned-${namespace.slice(0, 8)}.txt`, { type: 'text/plain', lastModified: 0 }), placeholders };
}

interface AttachmentOptions {
  enabled(): boolean;
  scrub(file: File): ReturnType<typeof scrubAttachment>;
  ...
  const intercept = (event: Event): void => {
    ...
    void (async () => {
      try {
        ...
        const pdfNote = results.some((r) => r.file.type === 'application/pdf')
          ? ' PDFs preserve page appearance with permanent redaction blocks; text is no longer selectable.' : '';
        options.status(`${results.length} sanitized file(s) handed to the site; ${placeholders.length} value(s) redacted. Check the attachment before sending.${pdfNote}`);
      } catch (error) {

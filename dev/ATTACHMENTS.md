...
per page; and 20 MB per output PDF. All files in a selection must pass.
Office files, images, archives, PDFs containing images, encrypted PDFs, binary
content, and invalid UTF-8 are blocked. Every PDF page must contain selectable
text. PDF parsing/rendering has a 60-second cancellation timer.
...
the upload is blocked instead of covering the whole line or guessing widths.
Sensitive rotated/skewed/vertical text is
blocked when reliable coverage is unavailable; page rotation is supported.
PDFs containing images need OCR before their pixels can be safely retained.
...
A failed or timed-out name scan blocks the selection. Detection remains
...
3. Select a text-based PDF and confirm the generated PDF retains its page
   layout with permanent redaction blocks. PDFs containing images or requiring
